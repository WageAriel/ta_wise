...
const filteredOutbounds = computed(() => {
    return props.outbounds.filter(ob => {
        const matchesSearch = 
            ...
            (ob.carrier && ob.carrier.toLowerCase().includes(searchQuery.value.toLowerCase()));
    ...
    form.post(route("admin.outbound.store"), {
        onSuccess: () => {
            Swal.fire({
                icon: "success",
                title: "Berhasil",
                text: "Catatan Outbound berhasil disimpan."
            });
...
const deleteOutbound = (id) => {
    Swal.fire({
        ...
        text: "Data outbound ini akan dihapus permanen! Catatan: Penghapusan data ini tidak mengembalikan stok inventory yang telah dikurangi.",
        icon: "warning",
            ...
            form.delete(route("admin.outbound.destroy", id), {
                onSuccess: () => {
                    Swal.fire({
                        icon: "success",
                        title: "Dihapus",
                        text: "Data outbound berhasil dihapus."
                    });
                }
            });
        }
...
<template>
    ...
    <AdminLayout>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6 flex flex-col sm:flex-row justify-between items-center gap-4">
            <div>
                <h2 class="text-lg font-semibold text-gray-700">Outbound Barang</h2>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            ...
            <div class="relative w-full md:flex-1">
                ...
                <input
                    v-model="searchQuery"
                    type="text"
                    placeholder="Cari No Outbound, Nama Tujuan, atau Ekspedisi..."
                ...
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="(ob, index) in filteredOutbounds" :key="ob.id_outbound" class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-gray-600">
                                <div v-if="ob.carrier" class="text-xs font-semibold text-indigo-600">{{ ob.carrier }}</div>
                                <div class="text-xs text-gray-500">
                                    {{ ob.nama_driver || '-' }} <span v-if="ob.plat_nomor">({{ ob.plat_nomor }})</span>
                                </div>
                            </td>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Ekspedisi / Carrier</label>
                                        <input 
                                            v-model="form.carrier" 
                                            type="text" 
                                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500" 
                                            placeholder="JNE, Pos, GoSend, Armada Sendiri..." 
                                        />
                                    </div>
                                </div>
                                ...
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">No. Resi / Surat Jalan Ekspedisi</label>
                                    <input 
                                        v-model="form.no_resi" 
                                        type="text" 
                                        class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500" 
                                        placeholder="Nomor resi/delivery note..." 
                                    />
                                </div>

                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Nota Timbang (URL)</label>
                                        <input 
                                            type="text" 
                                            v-model="form.nota_timbang"
                                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-xs" 
                                            placeholder="Masukkan URL/tautan nota..." 
                                        />
                                    </div>
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 mb-1">Surat Jalan (URL)</label>
                                        <input 
                                            type="text" 
                                            v-model="form.surat_jalan"
                                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-xs" 
                                            placeholder="Masukkan URL/tautan surat jalan..." 
                                        />
                                    </div>
                                </div>
                            </div>
                ...
                <div class="p-6 overflow-y-auto flex-1 min-h-0 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="bg-slate-50 p-5 rounded-xl border border-slate-100 space-y-4">
                            ...
                            <div class="grid grid-cols-3 gap-2 text-sm">
                                <div class="text-gray-500 font-medium">Ekspedisi</div>
                                <div class="col-span-2 text-gray-800 font-bold">: {{ selectedOutbound?.carrier || '-' }}</div>
                                ...
                                <div class="col-span-2 text-gray-800 italic">: {{ selectedOutbound?.catatan_pengiriman || '-' }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-slate-50 p-5 rounded-xl border border-slate-100 space-y-4">
                        ...
                        <div class="flex gap-4">
                            <div class="flex-1 p-3 border rounded-lg bg-white flex justify-between items-center">
                                <div>
                                    <div class="text-sm font-semibold text-gray-700">Nota Timbang</div>
                                    <div class="text-xs text-gray-400">{{ selectedOutbound?.nota_timbang_path ? 'Dokumen tersedia' : 'Tidak ada dokumen' }}</div>
                                </div>
                                <a 
                                    v-if="selectedOutbound?.nota_timbang_path"
                                    :href="formatPreviewUrl(selectedOutbound?.nota_timbang_path)" 
                                    target="_blank"
                            ...
                            <div class="flex-1 p-3 border rounded-lg bg-white flex justify-between items-center">
                                <div>
                                    <div class="text-sm font-semibold text-gray-700">Surat Jalan</div>
                                    <div class="text-xs text-gray-400">{{ selectedOutbound?.surat_jalan_path ? 'Dokumen tersedia' : 'Tidak ada dokumen' }}</div>
                                </div>
                                <a 
                                    v-if="selectedOutbound?.surat_jalan_path"
                                    :href="formatPreviewUrl(selectedOutbound?.surat_jalan_path)" 
                                    target="_blank"
                                    class="text-xs font-bold text-indigo-600 hover:text-indigo-800 flex items-center"
                                >
                                    Lihat File
